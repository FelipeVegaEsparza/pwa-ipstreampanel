...
Hoy todos los builds de cliente comparten los mismos iconos (`favicon`, `icon-192.png`, `icon-512.png`, `icon-maskable-512.png`, `apple-touch-icon.png`). Cada radio debería mostrar su propia marca tanto en la pestaña como en el icono de instalación de la PWA, sin duplicar assets compartidos ni tocar código del core por cliente.

## What Changes

- **Assets de marca por cliente**: nueva carpeta `clients/<nombre>/icons/` con los 5 archivos de branding (`favicon.png`, `icon-192.png`, `icon-512.png`, `icon-maskable-512.png`, `apple-touch-icon.png`).
...
- **Favicon PNG canónico**: `index.html` referencia `/favicon.png`; `public/favicon.png` es el default compartido y `clients/<nombre>/icons/favicon.png` lo reemplaza por cliente. Se deja de usar `favicon.svg`.
